...
const AdminNearBy = () => {
  ...
  const getHotels = () => {
    ...
    APIKit.get('/nearBy/findall').then(onSuccess).catch(onFailure);
  };
  const deleteHotels = id => {
    ...
    const onSuccess = ({data}) => {
      console.log(data);
      setLoading(false);
      setNearBy(nearBy.filter(item => item.id !== id));
    ...
    APIKit.delete(`nearBy/${id}`).then(onSuccess).catch(onFailure);
  };
  return (
          ...
          <View style={styles.item}>
            ...
            <View>
              <Text style={styles.title}>
                NearBy Place :{item.near_by_place}
              </Text>
